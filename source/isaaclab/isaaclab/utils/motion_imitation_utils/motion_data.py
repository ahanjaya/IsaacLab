...
class MotionData:
    ...
    def __init__(self, motion_data, frame_duration=None):
        ...
        if isinstance(motion_data, str):
            self.load(motion_data)
        elif isinstance(motion_data, list):
            self.loadFromArray(motion_data, frame_duration)
    ...
    def load(self, motion_file):
        ...
        logging.info(f"Loading motion from: {motion_file:s}")
        with open(motion_file) as f:
            ...
            logging.info(f"Loaded motion from {motion_file:s}.")

        return

    def save(self, filename):
        print(f"Saving motion to `{filename}`.")
        # Packs the data to be wrriten to disk in a dictionary.
        data = {
            "LoopMode": 1,
            "FrameDuration": self._frame_duration,
            "EnableCycleOffsetPosition": self._enable_cycle_offset_pos,
            "EnableCycleOffsetRotation": self._enable_cycle_offset_rot,
            "Frames": self._frames.tolist(),
        }

        with open(filename, "w") as fp:
            json.dump(data, fp)

    def loadFromArray(self, motion_frames, frame_duration):
        # TODO: Add parameters for loop mode and frame duration.
        self._loop_mode = LoopMode.Wrap
        self._enable_cycle_offset_pos = True
        self._enable_cycle_offset_rot = True
        self._frame_duration = frame_duration
        self._frames = np.array(motion_frames)
        self._postprocess_frames(self._frames)
    ...
    def get_frame_vel_size(self):
        ...
        """
        return self.get_frame_size() - self.POS_SIZE - self.ROT_SIZE + self.VEL_SIZE + self.ANG_VEL_SIZE
    ...
    def get_frames_vels(self):
        return self._frame_vels
    ...
    def _calc_frame_vels(self):
        ...
        for f in range(num_frames - 1):
            ...
            root_rot_diff_axis, root_rot_diff_angle = pose3d.QuaternionToAxisAngle(root_rot_diff)
