...
def QuaternionFromAxisAngle(axis, angle):
    ...
    if np.isclose(axis_norm, 0.0):
        raise ValueError(f"Axis vector may not have zero length: |v| = {axis_norm:f}, v = {axis}")
...
def QuaternionToAxisAngle(quat, default_axis=VECTOR3_Z, direction_axis=None):
    ...
    if not np.isclose(1.0, np.linalg.norm(quat)):
        raise ValueError(f"Quaternion should have unit length: |q| = {np.linalg.norm(quat):f}, q = {quat}")
    axis = quat[:3].copy()
    axis_norm = np.linalg.norm(axis)
    min_axis_norm = 1e-8
    if axis_norm < min_axis_norm:
        ...
        if not np.isclose(np.linalg.norm(axis), 1.0):
            raise ValueError(f"Axis vector should have unit length: |v| = {np.linalg.norm(axis):f}, v = {axis}")
    else:
        axis /= axis_norm
...
def IsRotationMatrix(m):
    ...
    if len(m.shape) != 2 or m.shape[0] < 3 or m.shape[1] < 3:
        raise ValueError(f"Matrix should be 3x3 or 4x4: {m.shape}\n {m}")
    rot = m[:3, :3]
