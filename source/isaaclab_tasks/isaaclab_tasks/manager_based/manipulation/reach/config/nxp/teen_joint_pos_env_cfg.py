...
# Pre-defined configs
##
from isaaclab_assets import NXP_TEEN_UPPER_BODY_CFG  # isort: skip
...
class ActionsCfg:
    ...
    # The identified DCMotorCfg actuator (see NXP_TEEN_UPPER_BODY_CFG) has very high stiffness (~220-500 Nm/rad)
    ...
    left_arm_action = mdp.RelativeJointPositionActionCfg(
        asset_name="robot",
        joint_names=NXP_TEEN_LEFT_ARM_JOINT_NAMES,
        scale=0.05,
        preserve_order=True,
...
@configclass
class NXPTeenReachEnvCfg(ReachEnvCfg):
    ...
    def __post_init__(self):
        ...
        # switch robot to nxp
        self.scene.robot = NXP_TEEN_UPPER_BODY_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
@configclass
class NXPTeenReachEnvCfg_PLAY(NXPTeenReachEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
